...
downloads every logged entry — date, hours, description, and a total — as a
PDF file, sorted oldest first. It's generated by hand-writing the PDF file
format directly in `includes/pdf.php` (no third-party library, no
Composer, no shell access), since none of those can be assumed on Etheron's
shared hosting. It uses the built-in Helvetica font all PDF viewers support
natively, so no fonts need to be embedded either. Long descriptions word-wrap
and the report paginates automatically once entries no longer fit on one
page.
