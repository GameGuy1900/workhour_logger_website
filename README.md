...
- **Geregistreerde uren** (Logged Entries) — every entry (date, hours,
  description) with a total, oldest first

To change the export's language, edit the label/heading strings directly in
`export_pdf.php` (and the `setPageLabelFormat()` call for the "Page X of Y"
footer) — nothing else needs to change, since `includes/pdf.php` just
