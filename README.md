...
downloads a full report as a PDF file:

- **Summary** — the current week's status and your all-time surplus totals
- **Weekly History** — every week (Week, Logged, Required, Status, Surplus,
  Rate, Pay), oldest first
- **Logged Entries** — every entry (date, hours, description) with a total,
  oldest first

It's generated by hand-writing the PDF file format directly in
`includes/pdf.php` (no third-party library, no Composer, no shell access),
since none of those can be assumed on Etheron's shared hosting. It uses the
built-in Helvetica font all PDF viewers support natively, so no fonts need
to be embedded either. Long descriptions word-wrap, and each section
paginates on its own — including repeating a table's header row on every
...
export_pdf.php         Streams all logged entries as a PDF download
